...
struct ExistingPointConnectionDetector: Sendable {
    ...
    mutating func update(
        ...
        if let lockedTarget {
            guard targets.contains(where: { $0.nodeID == lockedTarget.nodeID }),
                  simd_distance(position, lockedTarget.position) <= releaseDistance,
                  let startedAt else {
                cancelCandidate()
                return availableSnapshot
            }
        ...
        guard let nearestTarget = targets
            .map({ ($0, simd_distance(position, $0.position)) })
