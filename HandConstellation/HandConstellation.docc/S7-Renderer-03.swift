...
final class ConstellationRenderer {
  ...
  private func addLine(from start: SIMD3<Float>, to end: SIMD3<Float>) {
    ...
    line.orientation = simd_quatf(from: SIMD3<Float>(0, 1, 0), to: offset / length)
    lineContainer.addChild(line)
  }

  func reset() {
    for child in Array(pointContainer.children) { child.removeFromParent() }
    for child in Array(lineContainer.children) { child.removeFromParent() }
    hideCursor()
  }
}
