...
12. 점을 세 개 이상 만든 뒤 첫 점으로 돌아가 머무르면 새 점 없이 마지막 점과 첫 점을 연결한다.
13. 현재 가능한 조작과 도형 닫기 진행·완료·거부 이유를 제어창과 공간 피드백으로 안내한다.

## 3. 완성 경험

사용자는 앱의 기본 창에서 체험을 시작하고 Full Space로 진입한다. 앱이 손 추적 권한을 얻으면 검지 끝의 위치를 계속 관찰한다.

검지 끝이 작은 허용 반경 안에 일정 시간 머무르면 해당 월드 좌표에 별 점이 생성된다. 사용자가 손을 다른 위치로 옮겨 다시 머무르면 두 번째 점이 생성되고, 직전 점에서 새 점까지 선이 추가된다. 같은 방식으로 점과 선을 계속 이어 하나의 별자리를 만든다. 세 개 이상의 점을 만든 뒤 빛나는 첫 점으로 돌아가 머무르면 마지막 점에서 기존 첫 점까지 정확히 연결되어 닫힌 도형이 된다. 이때 첫 점 주변의 손 추적 오차는 스냅 반경으로 흡수하며 중복 점을 만들지 않는다.
...
- 선은 점이 추가된 순서대로 연결한다.
- 점이 세 개 이상일 때 시작점 스냅 영역에서 dwell을 완료하면 기존 시작점을 재사용해 마지막 선을 만든다.
- 닫기 동작은 새 점을 추가하지 않고 별자리를 닫힘 상태로 바꾼다.
- 닫힌 뒤 사용자가 완료 영역을 벗어나면 다음 점은 새 별자리를 시작한다.
...
- 닫기가 가능해지면 공간의 첫 점을 크기와 형태로 강조한다.
- 시작점 스냅 영역에서는 마지막 점과 첫 점 사이의 미리보기 선과 dwell 진행률을 보여 준다.
...
| 닫기 스냅 진입 거리 | 0.030m | 첫 점 주변의 손 추적 오차 흡수 |
| 닫기 스냅 해제 거리 | 0.040m | 경계에서 닫기 상태가 깜빡이는 현상 방지 |
...
닫기 판정은 일반 점 dwell과 분리한다. 세 점 이상일 때 첫 점을 닫기 대상으로 공개하고, 직전 점의 안정 반경을 벗어난 뒤 첫 점의 3cm 스냅 영역에 들어오면 0.8초 dwell을 시작한다. 진행 중에는 4cm 해제 반경을 사용해 경계 흔들림을 허용한다. 완료 뒤에는 첫 점을 기준으로 일반 dwell을 cooldown 상태로 전환해 같은 자리에서 새 별자리가 즉시 생기지 않게 한다.
...
| `ClosureDetector` | 시작점 스냅, 진입·해제 반경과 닫기 dwell 관리 |
| `FistHoldDetector` | 주먹 유지, 1회 전환과 손을 펼친 뒤 재활성화 관리 |
| `ConstellationModel` | 별자리별 점, 열림·닫힘 상태와 점·선분 규칙 관리 |
| `ConstellationRenderer` | RealityKit 점·선·커서·닫기 대상·미리보기 생성과 제거 |
...
- 세 점 미만에서는 닫기 입력이 활성화되지 않는다.
- 시작점 스냅 진입 반경에서 0.8초 머무르면 닫기 이벤트가 한 번 발생한다.
- 진입 반경을 벗어나도 해제 반경 안이면 닫기 진행률이 유지된다.
- 해제 반경을 벗어나거나 추적이 끊기면 닫기 진행이 취소된다.
- 닫을 때 기존 시작점이 재사용되고 점 개수는 증가하지 않는다.
- 닫힌 뒤 다음 점은 연결선 없이 새 별자리를 시작한다.
...
- 세 점 이상이면 첫 점이 닫기 대상으로 강조된다.
- 시작점에 접근하면 마지막 점에서 첫 점까지 미리보기 선이 나타난다.
- 닫기 진행·완료·너무 가까운 입력의 이유가 제어창에 안내된다.
- 삼각형을 닫으면 점 3개와 선 3개가 남고 중복된 네 번째 점이 생기지 않는다.
...
- 사용자가 시작점에 머무르지 않았는데 마지막 점과 첫 점을 자동 연결하는 동작
- 여러 개의 별자리 레이어 관리
- 앱 재실행 후 별자리 영구 저장
