...
> 2026-09-01 변경: 점을 세 개 이상 만든 뒤 시작점으로 돌아가 머무르면 기존 시작점을 재사용해 도형을 닫는다. 제어창의 상황별 안내와 공간의 시작점 강조·연결 미리보기로 이 조작을 설명한다.

> 2026-09-01 추가 변경: 현재 별자리의 모든 기존 점을 재연결 대상으로 확장하고 점·선을 그래프로 저장한다. 오른손 주먹은 중간 관절 굽힘으로 판정하며, 확인된 주먹을 1초 유지했을 때 그리기를 끄기만 한다. 런타임 점과 선은 흰색 단색으로 통일한다.

> 2026-09-01 최종 변경: 주먹 인식 기반 그리기 전환을 제거한다. 그리기 켜기와 끄기는 제어창 버튼으로만 수행하고 설명창에서도 주먹 관련 내용을 표시하지 않는다.
...
10. 창의 버튼으로만 그리기를 켜고 끈다.
11. 한 세션에서 여러 별자리를 만들며 서로 다른 별자리 사이에는 선을 연결하지 않는다.
12. 현재 별자리의 기존 점으로 돌아가 머무르면 새 점 없이 현재 출발점과 해당 점을 연결한다.
13. 연결된 기존 점을 새 출발점으로 삼아 계속 그릴 수 있으며 자기 연결과 중복 선은 거부한다.
14. 현재 가능한 조작과 기존 점 연결 진행·완료·거부 이유를 제어창과 공간 피드백으로 안내한다.

## 3. 완성 경험

사용자는 앱의 기본 창에서 체험을 시작하고 Full Space로 진입한다. 앱이 손 추적 권한을 얻으면 검지 끝의 위치를 계속 관찰한다.

검지 끝이 작은 허용 반경 안에 일정 시간 머무르면 해당 월드 좌표에 별 점이 생성된다. 사용자가 손을 다른 위치로 옮겨 다시 머무르면 새 점이 생성되고, 현재 출발점에서 새 점까지 선이 추가된다. 기존 점으로 돌아가 머무르면 새 점을 복제하지 않고 저장된 좌표로 선만 연결하며, 선택한 기존 점이 다음 선의 출발점이 된다. 점은 고유 ID를 가진 노드로, 선은 두 노드 ID의 엣지로 저장해 같은 선의 역방향 중복도 차단한다.
...
- 현재 별자리의 기존 점과 지나치게 가까운 좌표에는 새 점을 만들지 않는다.
...
- 새 점은 현재 출발점과 연결하고 새 점을 다음 출발점으로 삼는다.
- 현재 별자리의 기존 점 스냅 영역에서 dwell을 완료하면 해당 노드를 재사용해 선만 만든다.
- 기존 점 연결은 점 개수를 늘리지 않고 선택한 노드를 다음 출발점으로 삼는다.
- 자기 자신과 이미 연결된 두 노드 사이에는 새 선을 만들지 않는다.
- 이전에 마친 다른 별자리의 점은 연결 후보에서 제외한다.
...
- 제어창은 상시 조작법과 현재 해야 할 행동을 서로 분리해 보여 준다.
- 기존 점 스냅 영역에 들어오면 선택된 목표 하나를 크기와 투명도로 강조한다.
- 현재 출발점과 선택된 기존 점 사이의 미리보기 선과 dwell 진행률을 보여 준다.
- 점·선·커서·목표 피드백은 흰색 계열을 사용하고 상태는 크기와 투명도로 함께 구분한다.
- 너무 가까운 입력은 오류 상태로 바꾸지 않고 이동 방법을 안내한다.
...
| 기존 점 연결 진입 거리 | 0.030m | 기존 점 주변의 손 추적 오차 흡수 |
| 기존 점 연결 해제 거리 | 0.040m | 경계에서 목표가 깜빡이는 현상 방지 |
| 별 점 반지름 | 0.006m | 흰색 단색에서 식별 가능한 크기 |
| 연결선 반지름 | 0.0015m | 점보다 가볍고 가늘게 표현 |
...
기존 점 연결 판정은 일반 점 dwell과 분리한다. 현재 출발점에서 3cm 이상 벗어난 뒤 현재 별자리의 기존 점 스냅 영역에 들어오면 가장 가까운 목표를 고정하고 0.8초 dwell을 시작한다. 진행 중에는 4cm 해제 반경을 사용해 경계 흔들림과 목표 전환을 막는다. 완료 뒤에는 선택한 기존 점을 기준으로 일반 dwell을 cooldown 상태로 전환해 같은 자리에서 새 점이 즉시 생기지 않게 한다.
...
| `HandTrackingService` | ARKitSession 실행, 권한·지원 여부 확인, 검지 좌표 생성 |
| `DwellDetector` | 안정 반경, 시간 임계값, 재무장 상태 관리 |
| `ExistingPointConnectionDetector` | 모든 기존 점의 최근접 목표 고정, 진입·해제 반경과 연결 dwell 관리 |
| `ConstellationModel` | 별자리별 노드·엣지 그래프, 활성 노드와 중복 연결 규칙 관리 |
| `ConstellationRenderer` | RealityKit 흰색 점·선·커서·연결 목표·미리보기 생성과 제거 |
| `ControlView` | 시작, 종료, 초기화, 상시 조작법과 상황별 행동 안내 |
...
- 현재 별자리의 기존 점과 최소 거리보다 가까운 새 점은 거부된다.
- 가장 가까운 기존 점이 연결 목표로 선택되고 dwell 중에는 목표가 고정된다.
- 기존 점 스냅 진입 반경에서 0.8초 머무르면 연결 이벤트가 한 번 발생한다.
- 진입 반경을 벗어나도 해제 반경 안이면 연결 진행률이 유지된다.
- 해제 반경을 벗어나거나 추적이 끊기면 연결 진행이 취소된다.
- 기존 점 연결 시 점 개수는 증가하지 않고 선 개수만 증가한다.
- 같은 두 점 사이의 역방향 연결과 자기 연결은 거부된다.
- 연결한 기존 점에서 새 점을 만들면 해당 점에서 선이 출발한다.
...
- 어느 기존 점에 접근해도 해당 목표 하나가 강조된다.
- 기존 점에 접근하면 현재 출발점에서 목표까지 미리보기 선이 나타난다.
- 연결 진행·완료·이미 연결됨·너무 가까운 입력의 이유가 제어창에 안내된다.
- 기존 점에 연결하면 점은 늘지 않고 흰색 선만 정확히 하나 추가된다.
- 연결한 기존 점에서 새 가지를 계속 그릴 수 있다.
- 제어창의 버튼으로 그리기를 켜고 끌 수 있다.
- 점·선·커서와 연결 피드백이 흰색 단색으로 표시된다.
...
- 작업 경로: 프로젝트 루트
- Git 저장소: `https://github.com/82twj/visionOS_tutorial.git`
- 배포 주소: `https://82twj.github.io/visionOS_tutorial/documentation/handconstellation/`
...
전체 Xcode는 `/Applications/Xcode.app`에 설치되어 있지만 기본 개발자 디렉터리는 Command Line Tools를 가리킨다. 이후 명령줄 빌드에서는 Xcode 개발자 디렉터리를 명시하거나 사용자가 `xcode-select` 설정을 변경해야 한다. GitHub Actions는 저장소 이름 `visionOS_tutorial`을 hosting base path로 사용한다.
...
- 사용자가 기존 점에 머무르지 않았는데 자동으로 선을 연결하는 동작
- 서로 다른 별자리 그래프를 기존 점 선택으로 병합하는 동작
- 여러 개의 별자리 레이어 관리
- 앱 재실행 후 별자리 영구 저장
